...
receiptRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const paymentDDB: ebookPaymentDDB = result.Item as ebookPaymentDDB;
      //   if (!validatePaymentDDB(paymentDDB)) {
      //     return res.status(400).json({
      //       error:
      //         "Invalid Payment data. Ensure all required fields are present.",
      //     });
      //   }
      const payment: ebookPayment = ebookFromDDB(paymentDDB);
      ...
      //   generate pdf
      const pdfBuffer = await generateEbookReceiptPDF(payment);
      res.setHeader("Content-Type", "application/pdf");
      const filename = `receipt-${payment.orderId}.pdf`;
      res.setHeader("Content-Disposition", "attachment; filename= " + filename);
      res.send(pdfBuffer);
